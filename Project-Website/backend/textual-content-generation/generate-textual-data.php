    ...
    const askGPT =(q, page, i)=> {
        ...
        xhttp.onreadystatechange=()=> {
            if(xhttp.readyState == 4 && xhttp.status == 200) {
                loading.classList.add('d-none');
                let getImg = "";
                // let getImg = `<img id='gen-ai-${i}' src='https://citiwebcast.web.app/favicon.ico'> `;
...
?>
    ...
    let spPrompt<?php echo trim($i+1); ?> = "<?php echo trim($prompt); ?>Based on the above quote write a brief seo friendly html discription for putting it on website on <?php echo $_POST[$pageName]; ?>  section. provide a very short and crisp discription based on the above quote and must br related to <?php echo trim($_POST[$pageName]); ?> only. Also remove the irrelevent details if possible since we have already extrected other details.";
